...
def write_runtime_cluster_config():
    ...
    with open(hadoop_env_file_path, 'a') as hadoop_env_file:
        hadoop_env_file.write("export JAVA_HOME=" +
                              os.environ['JAVA_HOME'] + "\n")
        hadoop_env_file.write("export SPARK_MASTER_HOST=" + master_ip + "\n")
        hadoop_env_file.write("export AWS_CONTAINER_CREDENTIALS_RELATIVE_URI=" +
    ...
    file_data = file_data.replace(
        'minimum_allocation_mb', minimum_allocation_mb)
    file_data = file_data.replace(
        'maximum_allocation_mb', maximum_allocation_mb)
    file_data = file_data.replace(
        'minimum_allocation_vcores', minimum_allocation_vcores)
    file_data = file_data.replace(
        'maximum_allocation_vcores', maximum_allocation_vcores)
...
def get_ip_from_host(host_name):
    ...
    if counter == IP_WAIT_TIME and ip == '':
        raise Exception(
            "Exceeded max wait time of 300s for hostname resolution")
